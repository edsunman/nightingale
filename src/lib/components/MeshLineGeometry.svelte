<script lang="ts">
    ...
    export let shape : 'none' | 'taper' | ((p:number) => number) = 'none'
    ...
    const initializeGeometry = () => {
        ...
        for (let j = 0; j < pointCount; j++) {
            ...
            let width = shapeFunction ? shapeFunction(j / (pointCount - 1)) : 1
    ...
    const setPoints = (points: Vector3[]) => {
        ...
        setXYZXYZ(next, nextIndex, points[pointCount - 1].x, points[pointCount - 1].y, points[pointCount - 1].z)
